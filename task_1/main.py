...
def write_to_csv(file, data):
    with open(file, 'w', encoding='utf-8') as f_n:
        f_n_writer = csv.writer(f_n, quoting=csv.QUOTE_NONE)
        for nrow in data:
        ...
        with open(file, encoding='utf-8') as datafile:
            for row in datafile:
                ...
                if re.match('Изготовитель системы', row):
                    os_prod_list.append(re.search(r'(Изготовитель системы).\s*(\S*)', row).group(2))
                elif re.match('Название ОС', row):
                    os_name_list.append(re.search(r'(Windows\s\S*)', row).group(1))
                elif re.match('Код продукта', row):
                    os_code_list.append(re.search(r'(Код продукта).\s*(\S*)', row).group(2))
                elif re.match('Тип системы', row):
                    os_type_list.append(re.search(r'(Тип системы).\s*(\S*)', row).group(2))
